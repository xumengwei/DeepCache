package com.example.zmz.es_api23;
...
public class ES
{
    ...
    /*
        ...
        parameter                   alloc_parameter         vec_parameter           some required parameters, see datails in es.rs
        pre_movement                alloc_pre_movement      vec_pre_movement        matching movement of the previous picture
        match_result_es             alloc_res_es            vec_res_es              matching result(psnr) of the search
    ...
    */
    Allocation alloc_matchX,alloc_matchY,alloc_res_es,alloc_seq,alloc_res_recalculate,alloc_pre_movement,alloc_parameter,no_meaning;
    int[] vec_matchX,vec_matchY,vec_seq,vec_pre_movement,vec_parameter;
    double[] vec_res_recalculate,vec_res_es;
    void init(Context ctx,int img_sizex,int img_sizey,int blk_size,int steps,double threshold,int gap)
    {
        ...
        alloc_res_es = Allocation.createTyped(rs, tSquareFloat);
        ...
        vec_res_es=new double[blk_num_sq];
        ...
        ms.set_match_result_es(alloc_res_es);
    ...
    ES_RES run(Bitmap pre_pic,Bitmap cur_pic,ES_RES pre_es_res,boolean logging)// invoke es
    {
        long time_begin=SystemClock.uptimeMillis();
        ES_RES es_res=new ES_RES();

        es_res.PSNR=new double[blk_num_sq];
        vec_pre_movement[0]=pre_es_res.pre_movementX;
        vec_pre_movement[1]=pre_es_res.pre_movementY;
        vec_pre_movement[2]=pre_es_res.remX;
        vec_pre_movement[3]=pre_es_res.remY;
        ...
        ms.forEach_es(alloc_seq);// !!!important!!! invoke es function in rs

        alloc_matchX.copyTo(vec_matchX);// copy result to array
        alloc_matchY.copyTo(vec_matchY);
        alloc_res_es.copyTo(vec_res_es);
        ...
        es_res.pre_movementX=vec_pre_movement[0];
        vec_pre_movement[1]=(int) Math.rint(avg_movementY);
        es_res.pre_movementY=vec_pre_movement[1];
        ...
        es_res.remX=vec_pre_movement[2];
        es_res.remY=vec_pre_movement[3];
        alloc_pre_movement.copyFrom(vec_pre_movement);
        ...
        for(int j=0;j<blk_num_sq;j++)
        {
            ...
            es_res.PSNR[j]=vec_res_recalculate[j];
            if(psnr>threshold2)
                evaluation++;


        }
        es_res.evaluation=evaluation*1.0/blk_num_sq;
        long time_end=SystemClock.uptimeMillis();
        es_res.time=time_end-time_begin;
        if(logging==true)
        {
            System.out.println("x,y="+es_res.pre_movementX+" "+es_res.pre_movementY);
            System.out.println("delta t="+(time_end-time_begin));
            System.out.println("correct ratio="+TestBigDecimal.significand(es_res.evaluation,4));
        }
