package com.example.zmz.ds_api23;
 ...
 */
        ...
        import com.example.zmz.ds_api23.ScriptC_DS;

        import java.math.BigDecimal;
        import java.math.MathContext;
        import java.math.RoundingMode;
//import com.example.zmz.annotation_version.ScriptC_DS;
...
public class DS
{
    ...
    /*
        ...
        parameter                   alloc_parameter         vec_parameter           some required parameters, see datails in DS.rs
        pre_movement                alloc_pre_movement      vec_pre_movement        matching movement of the previous picture
        match_result_ds             alloc_res_ds            vec_res_ds              matching result(psnr) of the diamond search
    ...
    */
    Allocation alloc_matchX,alloc_matchY,alloc_res_ds,alloc_seq,alloc_res_recalculate,alloc_pre_movement,alloc_parameter,no_meaning;
    int[] vec_matchX,vec_matchY,vec_seq,vec_pre_movement,vec_parameter;
    double[] vec_res_recalculate,vec_res_ds;
    void init(Context ctx,int img_sizex,int img_sizey,int blk_size,int steps,double threshold,int gap)
    {
        ...
        alloc_res_ds = Allocation.createTyped(rs, tSquareFloat);
        ...
        vec_res_ds=new double[blk_num_sq];
        ...
        ms.set_match_result_ds(alloc_res_ds);
    ...
    DS_RES run(Bitmap pre_pic,Bitmap cur_pic,DS_RES pre_ds_res,boolean logging)// invoke DS
    {
        long time_begin=SystemClock.uptimeMillis();
        DS_RES ds_res=new DS_RES();

        ds_res.PSNR=new double[blk_num_sq];
        vec_pre_movement[0]=pre_ds_res.pre_movementX;
        vec_pre_movement[1]=pre_ds_res.pre_movementY;
        vec_pre_movement[2]=pre_ds_res.remX;
        vec_pre_movement[3]=pre_ds_res.remY;
        ...
        ms.forEach_ds(alloc_seq);// !!!important!!! invoke DS function in rs

        alloc_matchX.copyTo(vec_matchX);// copy result to array
        alloc_matchY.copyTo(vec_matchY);
        alloc_res_ds.copyTo(vec_res_ds);
        ...
        ds_res.pre_movementX=vec_pre_movement[0];
        vec_pre_movement[1]=(int) Math.rint(avg_movementY);
        ds_res.pre_movementY=vec_pre_movement[1];
        ...
        ds_res.remX=vec_pre_movement[2];
        ds_res.remY=vec_pre_movement[3];
        alloc_pre_movement.copyFrom(vec_pre_movement);
        ...
        for(int j=0;j<blk_num_sq;j++)
        {
            ...
            ds_res.PSNR[j]=vec_res_recalculate[j];
            if(psnr>threshold2)
                evaluation++;


        }
        ds_res.evaluation=evaluation*1.0/blk_num_sq;
        long time_end=SystemClock.uptimeMillis();
        ds_res.time=time_end-time_begin;
        if(logging==true)
        {
            System.out.println("x,y="+ds_res.pre_movementX+" "+ds_res.pre_movementY);
            System.out.println("delta t="+(time_end-time_begin));
            System.out.println("correct ratio="+TestBigDecimal.significand(ds_res.evaluation,4));
        }
